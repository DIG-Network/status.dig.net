...
export function classifyJsonRpc(outcome, opts = {}) {
  ...
  if (code < 200 || code >= 300) {
    return {
      status: STATUS.DOWN,
      ok: false,
      errorCode: code >= 500 ? ERROR_CODE.HTTP_5XX : ERROR_CODE.HTTP_4XX,
    };
  }
  ...
  if ('error' in body && body.error) {
    return {
      ...
      ok: false,
      errorCode: ERROR_CODE.RPC_ERROR,
      detail: { rpcError: body.error },
    };
  }
  if ('result' in body) {
    return { status: classify({ ok: true, latencyMs: outcome.latencyMs }, opts), ok: true };
...
export function classifyChainView(outcome, opts = {}) {
  ...
  if (typeof code !== 'number') {
    return {
      status: STATUS.DOWN,
      ok: false,
      errorCode: transportErrorCode(outcome),
      detail: { peakHeight: null, synced: null },
    };
  }
  if (code < 200 || code >= 300) {
    return {
  ...
  const peakHeight =
    state && state.peak && typeof state.peak.height === 'number' ? state.peak.height : null;
  if (peakHeight === null) {
    return {
      status: STATUS.DOWN,
      ok: false,
      errorCode: ERROR_CODE.NO_PEAK,
      detail: { peakHeight: null, synced: null },
    };
  }
  ...
  if (synced === false) {
    return {
      status: STATUS.DEGRADED,
      ok: false,
      errorCode: ERROR_CODE.NOT_SYNCED,
...
export function classifyPeakFreshness({ height, prevHeight, secondsSincePrev }) {
  ...
  if (secondsSincePrev > staleAfter) {
    return {
      status: STATUS.DEGRADED,
      ok: false,
      errorCode: ERROR_CODE.STALE_PEAK,
      detail: { advanced: 0, secondsSincePrev },
    };
  }
 ...
 *
 * NOTE: public/app.js#computeUptime is a byte-identical copy of this scoring.
 * The two are intentionally NOT shared: app.js is a classic non-module `<script>`
 * (no import), and this lib/ module is never shipped to the browser, so the
 * standalone static site cannot import it. Keep both definitions in lockstep —
 * change one, change the other (guarded by tests/uptime-parity.test.js).
 */
...
/** Minimal XML text-escaping for values interpolated into the Atom feed
 *  (entry titles/content built from system names, which come from our own
 *  targets.js — not untrusted input — but escaping is cheap and correct). */
...
/**
 * Build an Atom feed (public/feed.xml) of status TRANSITIONS from the rolling
 * per-system history (CLAUDE.md §6.6: ship a feed where content warrants it —
 * status.dig.net's incident/status history is exactly that). Pure/I/O-free
 * like the rest of this module: takes the history map (as shaped/returned by
 * appendHistory) and the latest statusDoc (for feed-level metadata + system
 * display names/descriptions), returns an XML string.
 *
 * Emits one <entry> per state CHANGE per system (e.g. up -> down, down -> up),
 * not one per sample — history.json samples every few minutes and would make
 * a per-sample feed useless noise. The very first sample of a series is a
 * baseline (no prior state to compare against) and never emits an entry.
 * Entries are ordered oldest-first to match Atom convention for a log feed.
 */
export function buildFeed(history, statusDoc) {
  const generatedAt = statusDoc.generatedAt;
  const byId = new Map(statusDoc.systems.map((s) => [s.id, s]));
  ...
  for (const id of Object.keys(history || {})) {
    ...
    for (let i = 1; i < series.length; i++) {
      ...
      transitions.push({ id, from: prev.status, to: cur.status, t: cur.t });
    }
  }
  transitions.sort((a, b) => new Date(a.t).getTime() - new Date(b.t).getTime());

  const entries = transitions.map(({ id, from, to, t }) => {
    const sys = byId.get(id);
    const name = xmlEscape((sys && sys.name) || id);
    const summary =
      to === STATUS.UP
        ? `${name} recovered (${from} -> up).`
        : `${name} is now ${to} (was ${from}).`;
    const entryId = `https://status.dig.net/#${xmlEscape(id)}-${xmlEscape(t)}`;
    return [
      '  <entry>',
      `    <id>${entryId}</id>`,
      `    <title>${name}: ${xmlEscape(to)}</title>`,
      `    <updated>${xmlEscape(t)}</updated>`,
      `    <link href="https://status.dig.net/" />`,
  ...
  return [
    ...
    '  <id>https://status.dig.net/</id>',
    '  <title>DIG Network Status</title>',
    '  <subtitle>Status-change history for the core DIG Network systems.</subtitle>',
    `  <updated>${xmlEscape(generatedAt)}</updated>`,
    '  <link rel="self" type="application/atom+xml" href="https://status.dig.net/feed.xml" />',
    '  <link rel="alternate" type="text/html" href="https://status.dig.net/" />',
    ...entries,
    '</feed>',
    '',
  ].join('\n');
